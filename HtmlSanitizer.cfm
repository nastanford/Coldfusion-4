<cffunction name="htmlSanitizer" returntype="string" access="public" output="false"
    hint="Remove XSS attacks from the user input">
    ...
    <cfset local.htmlTags = REMatchNoCase("<[^<]+>", arguments.userInput) />
    ...
    <cfloop list="#local.allowedTagList#" index="local.listTag">    
        <cfset local.inputTags = REMatchNoCase("<#local.listTag# [^<]+>", arguments.userInput) />
            ...
            <cfloop from="1" to="#ArrayLen(local.inputTags)#" index="local.inputTag">
                ...
                <cfif local.listTag EQ "a">
                    ...
                    <cfloop collection="#local.allowedAttributes#" item="local.attribute">
                        <cfset local.attributeValues = REMatchNoCase("#local.allowedAttributes["#local.attribute#"]#",local.inputTags[local.inputTag]) />
                        <!--- Remove the link if it's not clean --->
                        <cfif NOT arrayLen(local.attributeValues)>
                            <cfset arguments.userInput = ReplaceNoCase(arguments.userInput,local.inputTags[local.inputTag],"","all") />
                        <!--- If attribute is clean, use it to create clean a tag  --->
                        <cfelseif arrayLen(local.attributeValues)>
                            <cfset local.cleanAtag = listAppend(local.cleanAtag,local.attributeValues[arrayLen(local.attributeValues)]," ") />
                        </cfif>
                    </cfloop>
                    <!--- Close the tag and replace with user input --->
                    <cfset local.cleanAtag &= ">" />
                    <cfset arguments.userInput = ReplaceNoCase(arguments.userInput,local.inputTags[local.inputTag],local.cleanAtag,"all") />
                <!--- replace any other allowed tags with the clean version --->
                <cfelse>
                    <cfset arguments.userInput = reReplaceNoCase(arguments.userInput,"<#local.listTag# [^>]+>","<#local.listTag#>","all") />
                </cfif> 
            </cfloop>
        </cfif>
    </cfloop>